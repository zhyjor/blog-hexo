...
那么**视图的更新是怎么实现的呢**，我们来看父类`History中transitionTo()`方法的这么一段：
```js
...
```
**可以看出，它与push()的实现结构上基本相似，不同点在于它不是直接对`window.location.hash`进行赋值，而是调用`window.location.replace`方法将路由进行替换。**
